...
function CompanyCom() {
  ...
  const { getCompanyActiveJobs , getCompanyFinishedJobs } = useStateContext();
  ...
  const getActiveJobs = async () => {
    try {
      const data = await getCompanyActiveJobs(myUser?._id);
      setAll(data);
    } catch (e) {
      console.log(e);
    }
  ...
  const getFinishedJobs = async () => {
    try {
      const data = await getCompanyFinishedJobs(myUser?._id);
      setAll2(data);
    } catch (e) {
      console.log(e);
    }
  ...
  return (
    <div className="flex justify-center items-center flex-col xl:pl-32 lg:pl-24 md:pl-16 sm:pl-12 pl-4 xl:pr-32 lg:pr-24 md:pr-16 sm:pr-12 pr-4 md:pt-10 pt-5 md:pb-10 pb-7">
      ...
      <div className='mt-12 w-full'>
        ...
        <div className='flex justify-center'>
           <h4 className='md:text-lg pl-3 pr-3 pt-2 pb-2 rounded text-slate-600'>Jobs that are posted by you and are currently active to apply</h4>
        </div>
      ...
      <div className='mt-12 w-full'>
        ...
        <div className='flex justify-center'>
           <h4 className='md:text-lg pl-3 pr-3 pt-2 pb-2 rounded text-slate-600'>Jobs that are posted by you and are not active to apply</h4>
        </div>
        <div className='mt-2 border-[4px] border-slate-700 shadow-xl h-96 overflow-y-scroll no-scrollbar p-2'>
        {all2?.map((free) => (
            <JobCard title={free?.title} desc={free?.description} cost={free?.expected_cost} pId={free?.pId} image={free?.image}/>
